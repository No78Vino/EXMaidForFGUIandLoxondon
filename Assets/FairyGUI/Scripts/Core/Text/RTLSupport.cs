...
namespace FairyGUI
{
    public class RTLSupport
    {
        ...
        private static readonly Dictionary<int, char[]> mapping = new();

        private static readonly List<char> listFinal = new();
        private static readonly List<string> listRep = new();
        private static readonly StringBuilder sbRep = new();
        private static readonly StringBuilder sbN = new();
        private static readonly StringBuilder sbFinal = new();
        private static readonly StringBuilder sbReverse = new();

        public static bool IsArabicLetter(char ch)
        {
            if (ch >= 0x600 && ch <= 0x6ff)
            {
                if ((ch >= 0x660 && ch <= 0x66D) ||
                    (ch >= 0x6f0 && ch <= 0x6f9)) // 标准阿拉伯语数字和东阿拉伯语数字 [2019/3/1/ 17:45:18 by aq_1000]
                    return false;
                return true;
            }
            ...
            if (ch >= 0xfb50 && ch <= 0xfc3f)
                return true;
            if (ch >= 0xfe70 && ch <= 0xfefc) return true;
        ...
        private static DirectionType _GetDirection(char uc, char nextChar, DirectionType ePre, DirectionType eBase)
        {
            ...
            else if (_IsNeutrality(uc)) // 中立方向字符，方向就和上一个字符一样 [2018/3/24 16:03:27 --By aq_1000]
            {
                ...
                {
                    if (char.IsNumber(
                            nextChar)) // 数字都是弱LTR方向符，开头中立字符后面紧跟着数字的话，中立字符方向算文本主方向 [IsDigit()只是0-9] [2018/12/20/ 16:32:32 by aq_1000]
