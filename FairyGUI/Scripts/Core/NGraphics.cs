...
namespace FairyGUI
{
    ...
    public class NGraphics : IMeshFactory
    {
        ...
        private int _customMatarial; //0-none, 1-common, 2-support internal mask, 128-owns material
        ...
        private List<byte> _alphaBackup; //透明度改变需要通过修改顶点颜色实现，但顶点颜色本身可能就带有透明度，所以这里要有一个备份
        ...
        public void ToggleKeyword(string keyword, bool enabled)
        {
            ...
            else
            {
                if (_shaderKeywords != null)
                {
                    var i = Array.IndexOf(_shaderKeywords, keyword);
                    if (i != -1)
                    {
        ...
        public void Tint()
        {
            ...
            var colors = vb.colors;
#endif
            for (var i = 0; i < vertCount; i++)
            {
                Color32 col = color;
                col.a = (byte)(_alpha * (hasAlphaBackup ? _alphaBackup[i] : 255));
                colors[i] = col;
            }
        ...
        internal void _PreUpdateMask(UpdateContext context, uint maskId)
        {
            ...
            if (_manager != null)
            {
                ...
                var mat = _manager.GetMaterial((int)MaterialFlags.AlphaMask | _materialFlags, BlendMode.Normal, maskId);
        ...
        private void UpdateMeshNow()
        {
            ...
            if (_texture.rotated)
            {
                var xMin = _texture.uvRect.xMin;
                var yMin = _texture.uvRect.yMin;
                var yMax = _texture.uvRect.yMax;
                float tmp;
                for (var i = 0; i < vertCount; i++)
                {
                    var vec = vb.uvs[i];
            ...
            if (_vertexMatrix != null)
            {
                ...
                var center = new Vector3(camPos.x, camPos.y, 0);
                center -= _vertexMatrix.matrix.MultiplyPoint(center);
                for (var i = 0; i < vertCount; i++)
                {
                    var pt = vb.vertices[i];
        ...
        {
            var rect = texture.GetDrawRect(vb.contentRect);
